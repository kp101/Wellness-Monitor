...
@router.subscribe(MQTT_TOPIC_KITCHEN)
async def handle_kitchen(message):
    msg = message.payload.decode('utf-8')
    logging.debug(f"{MQTT_TOPIC_KITCHEN}:{msg}.")
    try:
        if msg=="terminate":
            pygame.mixer.quit()
